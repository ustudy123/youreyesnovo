...
--
-- Por que a homologação precisa disto ANTES de rodar a bateria de tela:
-- a guarda de cobertura (npm run qa:cobertura-e2e) reprova a corrida se um
-- it() novo não tiver caso documentado. Sem estas linhas, os it() de férias
-- seriam lidos como "inventados".
--
-- Regras da casa atendidas: só CRIA documentação (não altera/apaga dado —
-- dispensa backup); idempotente (ON CONFLICT DO NOTHING, pode rodar 2x);
-- bloco DO com EXCEPTION por segurança (nunca aborta a transação inteira);
-- sem BEGIN/COMMIT; termina com UMA conferência SELECT.
-- =====================================================================
...
BEGIN
  ...
  IF v_mod IS NULL THEN
    RAISE NOTICE 'Modulo jornada-rotina/ferias nao existe nesta base. Casos FERIAS-TELA NAO inseridos — documente os modulos QA antes.';
  ELSE
    ...
    RAISE NOTICE 'Casos e ponte FERIAS-TELA aplicados (idempotente).';
  END IF;

EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'Erro ao aplicar FERIAS-TELA: %', SQLERRM;
END $ferias$;

-- ── Conferência (o editor mostra só o último resultado) ──────────────
SELECT
  (SELECT count(*) FROM public.qa_casos_teste     WHERE codigo LIKE 'FERIAS-TELA-%') AS casos_ferias_tela,
  (SELECT count(*) FROM public.qa_cobertura_e2e   WHERE codigo LIKE 'FERIAS-TELA-%') AS pontes_ferias_tela,
  (SELECT count(*) FROM public.qa_casos_teste c
      JOIN public.qa_modulos m ON m.id = c.modulo_id
     WHERE m.path = 'jornada-rotina/ferias' AND c.nivel = 'e2e')                     AS e2e_no_modulo_ferias;
-- Esperado após aplicar: casos_ferias_tela = 10, pontes_ferias_tela = 10.
