-- =====================================================================
-- DADOS FISCAIS DA YOUREYES · SCRIPT DE ENTREGA
-- Cole no SQL Editor de cada ambiente (Teste → Homologação → Produção).
-- Cria o cadastro único dos dados fiscais/contábeis da YourEyes (razão
-- social, CNPJ, endereço, contato, representante, foro), a leitura pública
-- sem dado sensível e liga esses dados ao Contrato de Parceria. Só cria e
-- ajusta o texto do contrato v2 (sem aceites em produção). Idempotente.
-- Depois de aplicar: preencher em SuperAdmin › Dados da YourEyes.
-- Corresponde à migration 20260904190000_youreyes_dados_fiscais.sql.
-- =====================================================================
...
WITH t AS MATERIALIZED (SELECT count(*) AS n FROM public.youreyes_empresa),
     f AS MATERIALIZED (SELECT count(*) AS n FROM pg_proc p JOIN pg_namespace ns ON ns.oid = p.pronamespace WHERE ns.nspname='public' AND p.proname IN ('superadmin_youreyes_empresa_salvar','youreyes_empresa_publica','parceiro_contrato_render')),
     c AS MATERIALIZED (SELECT bool_or(html LIKE '%{{YE_RAZAO_SOCIAL}}%') AS contrato_usa_dados FROM public.parceiro_contratos_versoes WHERE vigente),
     p AS MATERIALIZED (SELECT (SELECT cnpj FROM public.youreyes_empresa WHERE id = 1) IS NOT NULL AS cnpj_preenchido)
SELECT CASE WHEN t.n = 1 AND f.n = 3 AND c.contrato_usa_dados THEN 'OK — dados fiscais da YourEyes prontos' || CASE WHEN p.cnpj_preenchido THEN '' ELSE ' (preencher em SuperAdmin › Dados da YourEyes)' END
            ELSE 'ATENÇÃO — confira as colunas ao lado' END AS resultado,
       t.n AS registro, f.n || '/3' AS funcoes, c.contrato_usa_dados, p.cnpj_preenchido, NULL::text AS erro_tecnico
FROM t, f, c, p;
