...
-- migrations 20260911220000, 221000, 222000, 224000 e 230000 (a de
...
WITH f AS MATERIALIZED (
  ...
  FROM unnest(ARRAY['001','002','003','004','005','006','007','008','009','010','011','012','013','014']) AS c(codigo)
), qr AS MATERIALIZED (
...
SELECT CASE WHEN f.buscar = 1 AND f.cadastro = 1 AND f.avaliar = 1 AND f.portal = 1 AND f.vitrine_publica = 1 AND f.contestacao = 1 AND f.antiga_com_pii = 0
             ...
             AND c.politica_antiga_removida AND c.pii_fechada AND c.casos_qa >= 17 AND qr.passaram = qr.total
