-- =========================================================
-- SCRIPT DE ENTREGA — Culpa reciproca: o motivo que faltava
-- Projeto: PRODUCAO (colar no SQL Editor)
--
-- O PROBLEMA (achado pelo caso de teste DEC13-061)
--   "Culpa reciproca" nao existia entre os motivos de rescisao. Quem
--   opera o desligamento tinha duas saidas, ambas erradas: lancar como
--   justa causa (paga ZERO — a empresa fica devedora) ou como dispensa
--   comum (paga INTEIRO — a empresa perde dinheiro).
--
-- O QUE A LEI DIZ
--   Reconhecida a culpa reciproca, as verbas devidas na dispensa sem
--   justa causa sao pagas pela METADE (CLT, art. 484; Sumula 14 do TST).
--   Alcanca aviso previo, 13o proporcional, ferias proporcionais e a
--   multa do FGTS (40% viram 20%). Saldo de salario e ferias VENCIDAS
--   nao entram na metade: sao direito ja adquirido.
--
-- O QUE ESTE SCRIPT FAZ
--   1. acrescenta CULPA_RECIPROCA ao vocabulario de motivos;
--   2. aplica a metade no 13o da rescisao, com o fundamento na memoria;
--   3. ensina a trava a recusar os DOIS jeitos de errar: 13o na justa
--      causa e 13o INTEIRO na culpa reciproca.
--
-- NAO altera nenhuma rescisao ja gravada — so cria o motivo novo e
-- muda regra para o que entrar de agora em diante. Por isso nao ha
-- copia de seguranca a fazer. Idempotente.
--
-- ATENCAO: a tela da rescisao so oferece o motivo novo depois do
-- Publicar no Lovable. Este script prepara o banco; a tela vem depois.
-- =========================================================
...
-- ── Conferencia final ─────────────────────────────────────────────────
SELECT 'motivo CULPA_RECIPROCA no vocabulario' AS item,
 ...
 UNION ALL
SELECT 'o 13o da rescisao paga metade na culpa reciproca',
       CASE WHEN position('metade_por_culpa_reciproca' in p.prosrc) > 0 THEN 'OK' ELSE 'FALTOU' END,
       CASE WHEN position('metade_por_culpa_reciproca' in p.prosrc) > 0 THEN NULL
            ELSE 'decimo_terceiro_da_rescisao nao foi substituida' END
  FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
 WHERE n.nspname='public' AND p.proname='decimo_terceiro_da_rescisao'
 UNION ALL
SELECT 'a trava recusa o 13o INTEIRO na culpa reciproca',
       CASE WHEN position('CULPA_RECIPROCA' in p.prosrc) > 0 THEN 'OK' ELSE 'FALTOU' END,
       CASE WHEN position('CULPA_RECIPROCA' in p.prosrc) > 0 THEN NULL
            ELSE 'decimo_terceiro_rescisao_valida nao foi substituida' END
  FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
 WHERE n.nspname='public' AND p.proname='decimo_terceiro_rescisao_valida'
 UNION ALL
SELECT 'rescisoes ja gravadas que podem ser culpa reciproca (conferir a mao)',
       CASE WHEN count(*) = 0 THEN 'OK' ELSE 'CONFERIR' END,
       CASE WHEN count(*) = 0 THEN NULL
            ELSE count(*)::text || ' rescisao(oes) por justa causa no ultimo ano: se alguma foi, '
                 || 'na verdade, culpa reciproca reconhecida, ela precisa ser revista a mao — '
                 || 'este script vale para o que entrar de agora em diante' END
  FROM public.folha_rescisoes
 WHERE tipo_rescisao::text = 'DISPENSA_COM_JUSTA_CAUSA'
   AND data_desligamento >= CURRENT_DATE - 365
 ORDER BY 2 DESC, 1;
