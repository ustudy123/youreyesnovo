-- =========================================================
-- 13º Salário — política do adiantamento configurável e média
-- das horas extras pela Súmula 347 do TST
--
-- DUAS DECISÕES DO DONO DO PRODUTO (03/09/2026):
--   (a) as duas leituras do adiantamento são defensáveis; a empresa
--       escolhe qual seguir;
--   (b) no mais, seguir o entendimento mais protetivo, para blindar a
--       empresa de passivo trabalhista.
--
-- ── 1. ADIANTAMENTO: o parâmetro existia e não fazia nada ────────────
-- decimo_terceiro_config.adiantamento_base foi criado e documentado na
-- Entrega 1, mas o cálculo nunca o leu: fazia sempre metade do 13º
-- proporcional. Falha minha. Agora ele vale de verdade:
--
--   'proporcional_apurado' (padrão) — metade do 13º proporcional apurado
--       (salário + médias, na fração dos avos). É a prática de mercado:
--       para quem tem menos de 12 avos, evita adiantar mais do que o
--       próprio 13º devido.
--
--   'remuneracao_mes_anterior' — metade da remuneração do mês anterior
--       ao pagamento, que é a letra do art. 2º da Lei 4.749/1965
--       ("metade do salário recebido pelo respectivo empregado no mês
--       anterior"). Lê a folha do mês anterior; sem folha lançada, cai
--       no salário do cadastro e avisa na memória.
--
-- Ambas produzem 13º TOTAL idêntico: o adiantamento é só antecipação, e
-- a 2ª parcela acerta a diferença. O que muda é quanto entra em novembro.
--
-- ── 2. MÉDIA DAS HORAS EXTRAS: Súmula 347 do TST ─────────────────────
-- Até aqui a média somava os VALORES pagos no ano e dividia pelos avos.
-- A Súmula 347 do TST consolida método diferente para o reflexo de horas
-- extras habituais: apura-se a MÉDIA FÍSICA (quantidade de horas
-- efetivamente prestadas) e aplica-se o valor do salário-hora da ÉPOCA
-- DO PAGAMENTO da verba. Quando houve aumento salarial no ano, a média
-- por valores históricos paga a MENOS — e é diferença que vira ação.
--
-- O Ponto já guarda o que falta: ponto_diario.horas_extras_50_minutos e
-- horas_extras_100_minutos, separados, e a CCT guarda os percentuais
-- (ponto_cct_config) e a jornada semanal.
--
-- Padrão 'fisica', com queda automática para 'valores' quando não há
-- ponto no ano — e a memória sempre diz qual método foi usado.
--
-- Rubricas que NÃO são hora extra (comissão, adicionais) seguem pela
-- média de valores: a Súmula 347 é sobre horas extras. Para o sistema
-- saber quais rubricas vêm do ponto e não contá-las duas vezes,
-- folha_rubricas ganha a marca he_do_ponto.
--
