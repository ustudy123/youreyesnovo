-- =========================================================
-- 13º Salário — Entrega 4: provisão contábil e rescisão
--
-- DOIS PROBLEMAS, ambos "a tabela existe e nada a alimenta":
--
-- 1) PROVISÃO (CA-009). folha_provisoes tem o desenho certo — tipo,
--    encargos, reversão — e NENHUMA função a alimenta: a provisão do 13º
--    é lançada à mão, quando alguém lembra. Provisão por regime de
--    competência não é enfeite: o custo nasce 1/12 por mês, admissões e
--    desligamentos a ajustam, e o contador precisa conciliar
--    provisionado x pago no fim do ano. À mão, ela desalinha da folha no
--    primeiro mês esquecido e o balancete de dezembro leva o susto do
--    ano inteiro de uma vez.
--
-- 2) RESCISÃO (RN-009). folha_rescisoes tem o campo
--    decimo_terceiro_proporcional e nenhuma regra por trás: justa causa
--    com 13º proporcional era aceito, porque a regra vivia só no React.
--    E nada ligava a rescisão ao 13º já pago: o adiantamento não era
--    conferido nas verbas e o desligado reaparecia na rodada anual.
--
-- ENTREGA:
--   * decimo_terceiro_provisionar(competencia) — provisiona 1/12 do 13º
--     de cada vínculo ativo, com INSS patronal e FGTS, idempotente por
--     competência; reverte a provisão de quem foi desligado ou já teve o
--     13º pago;
--   * decimo_terceiro_conciliar_provisao(ano) — o que o contador pede:
--     provisionado x pago, mês a mês, com a diferença;
--   * trava de banco na rescisão: justa causa não leva 13º proporcional
--     (a culpa recíproca, com 50%, é tratada no módulo de Desligamento —
--     DESL-035, Súmula 14 do TST);
--   * decimo_terceiro_da_rescisao(rescisao) — apura o proporcional pelo
--     motivo, desconta o adiantamento já pago e devolve a memória;
--   * o lote anual passa a PULAR quem foi desligado antes do pagamento:
--     as verbas do desligado saem na rescisão, não na folha de dezembro.
--
-- Requisitos YE-DP-13-001: RF-005 (provisão), RN-009, CA-009, seção 20.
-- Casos: DEC13-051, DEC13-060.
-- =========================================================
