-- =====================================================================
-- PROGRAMA DE PARCEIROS · CONTRATO COM ASSINATURA ELETRÔNICA COMPLETA
--
-- Antes: o parceiro "aceitava" o contrato com um clique (clickwrap) e a
-- casa gravava uma cópia já marcada como assinada na tela de Contratos.
-- Agora: o aceite do cadastro só manifesta a intenção. A assinatura de
-- verdade acontece no MESMO fluxo dos contratos existentes do SuperAdmin
-- (/assinar-contrato/:token): assinatura manuscrita na tela, selfie ao
-- vivo, IP, navegador, geolocalização, data/hora e hash SHA-256 do texto
-- (Lei 14.063/2020, art. 4º, II — assinatura eletrônica avançada; MP
-- 2.200-2/2001, art. 10, § 2º). O contrato só conta como aceito pelo
-- parceiro depois que essa assinatura é registrada.
--
-- O texto é gerado por parceiro, com a qualificação das DUAS partes (a
-- YourEyes vem de youreyes_empresa; o parceiro, do cadastro), e formatado
-- no padrão ABNT (fonte serifada 12 pt, entrelinha 1,5, texto justificado,
-- margens 3 cm superior/esquerda e 2 cm inferior/direita em A4, títulos
-- numerados, recuo de parágrafo). A cópia gerada fica em
-- contratos_assinaturas.html_assinado e é a que a tela de Contratos do
-- SuperAdmin exibe e imprime.
--
-- Idempotente. Só cria/substitui funções e ajusta flags do modelo
-- 'parceria' na tela de Contratos (não apaga nem altera assinatura feita).
-- =====================================================================
