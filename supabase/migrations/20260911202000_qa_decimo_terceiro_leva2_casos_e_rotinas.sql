-- =========================================================
-- QA — 13º Salário: 2ª leva de casos documentados (Documentação de testes)
--
-- MÉTODO: revisão de TODA a documentação de testes do módulo (17 casos da
-- 1ª leva) contra a legislação e o documento YE-DP-13-001, procurando o
-- que a lei exige e ninguém testava. Saíram 14 casos novos, cada um com
-- base legal própria e rotina executável no motor de QA.
--
-- O QUE ESTA LEVA COBRE E A ANTERIOR NÃO COBRIA:
--   · a projeção do aviso prévio indenizado nos avos (DEC13-004);
--   · a fronteira exata dos 15 dias, dia a dia (DEC13-005);
--   · acidente do trabalho, que não pode pesar contra o 13º (DEC13-006);
--   · a média FÍSICA das horas extras, da Súmula 347 (DEC13-022);
--   · adicionais habituais na base — noturno, insalubridade,
--     periculosidade (DEC13-023);
--   · a escolha da política do adiantamento e o pedido fora de janeiro
--     (DEC13-034 e DEC13-035);
--   · faixas, bordas e teto do INSS (DEC13-043);
--   · a reversão da provisão de quem foi desligado (DEC13-052);
--   · os motivos de rescisão que a 1ª leva não separava: culpa recíproca,
--     pedido de demissão e falecimento (DEC13-061, 062 e 063);
--   · o lote clicado duas vezes (DEC13-072);
--   · a antecipação da data-limite para o último dia útil (DEC13-073).
--
-- As rotinas seguem o padrão da casa: somente leitura sobre o sistema, e
-- quando precisam de dado montam um vínculo fictício (CPFs 900.000.0XX
-- com dígito válido) e apagam o próprio rastro ao final.
-- =========================================================
